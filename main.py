...
####
def load_ppi(fname='./polypharmacy/bio-decagon-ppi.csv'):
    fin = open(fname)
    ...
    return net, len(edges),len(nodes)

def load_combo_se(fname='./polypharmacy/bio-decagon-combo.csv'):
    combo2stitch = {}
    ...
    for line in fin:
        ...
        se2name[se] = se_name
        drug[stitch_id1,se_name ,stitch_id2]= [(0,0)]
    fin.close()
    g = dgl.heterograph(drug)
...
def load_targets(fname='./polypharmacy/bio-decagon-targets.csv'):
...
net, gene_edges,n_genes = load_ppi()
drug2gene = load_targets()
drug,sename= load_combo_se()
...
gene_drug_adj = drug2gene.adjacency_matrix()
...
for key, value in sename.items():
    mat = drug.adjacency_matrix(etype=value)
    if sum(sum(mat[i]) for i in range(len(mat))) > 500:
